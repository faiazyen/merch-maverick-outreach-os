...
const data = JSON.parse(fs.readFileSync(__dirname + "/batch2-leads.json", "utf-8"));
...
const CONFIDENCE_LABELS = {
  found_on_site: "found on site",
  pattern_guessed_mx_verified: "pattern guessed",
};
...
function buildEntry(row, index, total) {
  ...
  const confidenceLabel = CONFIDENCE_LABELS[row.email_confidence] || row.email_confidence;
  const f5 = fieldLine("Email confidence", confidenceLabel);
  if (f5) paras.push(f5);
...
children.push(new Paragraph({
  ...
  alignment: AlignmentType.CENTER,
  children: [new TextRun({ text: "Outreach Batch 2: 1,000 Personalized Emails" })],
  spacing: { after: 400 },
}));
children.push(new Paragraph({
  alignment: AlignmentType.CENTER,
...
Packer.toBuffer(doc).then((buffer) => {
  fs.writeFileSync(__dirname + "/Merch-Maverick-Outreach-Batch2.docx", buffer);
  console.log("Done. Wrote Merch-Maverick-Outreach-Batch2.docx");
});
